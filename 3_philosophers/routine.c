...
/*   Updated: 2022/10/24 15:27:15 by pnolte           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	sleepy_philo(t_philo *phi)
{
	...
	if (phi->timer_eat * 2 + phi->timer_sleep >= phi->timer_die
		&& phi->nbr_phi % 2 != 0)
	{
		print_manager('t', phi);
		my_sleep(phi->timer_die - phi->curr_run_mili, phi);
...
void	*routine(void *arg)
{
	...
	while (phi->slk->death == false)
	{
		...
		if (phi->nbr_phi % 2 != 0)
			my_sleep(phi->timer_eat, phi);
		if (phi->nbr_times_phi_eat != 0
			&& phi->count_phi_eat == phi->nbr_times_phi_eat)
			return (NULL);
		pthread_mutex_lock(&phi->slk->lock);
	}
	pthread_mutex_unlock(&phi->slk->lock);
